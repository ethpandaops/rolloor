...
# examples/kurtosis is the one exception: it shows a real devnet wired up.
set -euo pipefail
...
if hits=$(git grep -nIiwE "(${words})" -- . ':!scripts/lint-words.sh' ':!examples/kurtosis' ':!go.sum' ':!internal/ui/static/htmx.min.js'); then
